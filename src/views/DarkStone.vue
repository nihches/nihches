...
<template>
  <main>
    ...
    <stats>
      <template #izM id="izDM"> {{ dDm }}&#x26CB;</template>
    ...
    <Spaces v-for="space in spaces" :id="'space'+space.id">
      <template #spot>
          <div :id="'spot'+space.id" >
            <img :id="'simg'+space.id" class="space_img" :src="`/nihches/space.png`" @click="spaceClick(space.id)" >
          </div>
        </template>
        ...
        <template #selectNav>
          <div class="selection_meni" v-if="sCelestial || sCreate || sInventory === true">
            ...
            <div class="selection_content" v-if="sCreate">
              ...
              <div v-for="i in whm" :id="'wh'+i">              
                <div class="s_item" @click="whiteHole(i)">
                  <div class="s_option">
                    <img class="icon_option cr_meni" :src="`/nihches/wh`+(i-1)+`.jpg`">
                    <div class="itemname"><b>WhiteHole {{i-1}}</b></div>
                    <div class="dis_option2" id="dBdsi"></div>
                    <div class="dis_option" id="dBds">{{((0.1*(Math.PI)**i)).toFixed(2)}}&#x26CB;</div>
                  </div>
                </div>
              </div>
              <div class="s_item" @click="whUP()">
                <div class="s_option">
                  ...
                  <div class="itemname"><b>WhiteHole {{whm}} </b></div>
                  <div class="dis_option2" id="dBdsi"></div>
                  <div class="dis_option" id="dBds">{{((0.1*(Math.PI)**(whm+2))).toFixed(2)}}&#x26CB;</div>
                </div>
              </div>
            </div>
            <div class="selection_content" v-if="sCelestial">
              <div class="title_meni">Create celestial</div>
              <div v-for="item in invSun">
                <div v-if="item.name !== 'cc' " class="s_item" @click="itemClick(item.name)">
                  <div>
                    ...
                    <div class="itemname"><b>{{ item.title }}</b></div>
                    <div class="dis_option">{{ item.text }}</div>
                  </div>
                </div>
                <div v-else-if="item.name === 'cc' && dm_p>=1 && dm >=1" class="s_item" @click="itemClick(item.name)">
                  <div>
                    <img class="icon_option" :src="`/nihches/`+item.name+`.jpg`" />
                    <div class="starNum">{{ invSun[item.id].qnt }}</div>
                    <div class="itemname"><b>{{ item.title }}</b></div>
            ...
            <div class="selection_content" v-if="sInventory">
              ...
              <div v-for="item in invSun">
                <div class="s_item" id="s_bds" @click="itemClick(`inv`+item.name)" v-if="item.qnt >0">
                  <div>
...
export default {
  ...
  data() {
    return{
      ...
      invSun: JSON.parse(localStorage.getItem("invSun")) || [
        ...
        {id: 1, name: "ys", title: "Yellow Star", qnt: 0, cost: 100, value: 110, text: "100H+1BD"},
        {id: 2, name: "bg", title: "Blue Giant", qnt: 1, cost: 1000, value: 1100, text: "1kH+1YS"},
        {id: 3, name: "bh", title: "Black hole", qnt: 0, cost: 10, value: 10000, text: "10BG"},
        {id: 4, name: "cc", title: "Celestial Coin", qnt: 10, cost: 10, value: 4796, text: ""}
      ],
    }
  },
  computed: {
  ...
  methods: {
        ...
        switch(this.tmp_inv){
          ...
          case "wh":
            if(this.dm>= this.wh_size){
              ...
              this.spaces[id] = {id: id, state:"wh", value: this.wh_size*100, end: this.wh_size, item: this.whs}
                document.getElementById("simg"+id).setAttribute("src", `/nihches/wh`+ (this.whs) +`.jpg`)
                this.saveTime()
            }
            break
          case "invbds":
          if(this.invSun[0].qnt >=1){
            this.invSun[0].qnt --
            this.spaces[id] = {id: id, state:"sun", value: this.invSun[0].value, end: this.invSun[0].value, item: "bds"}
                document.getElementById("simg"+id).setAttribute("src", `/nihches/bds.jpg`)
                this.saveTime()
          }
          break
          ...
          if(this.invSun[1].qnt >=1){
            this.invSun[1].qnt --
            this.spaces[id] = {id: id, state:"sun", value: this.invSun[1].value, end: this.invSun[1].value, item: "ys"}
                document.getElementById("simg"+id).setAttribute("src", `/nihches/ys.jpg`)
                this.saveTime()
          }
          break
          ...
          if(this.invSun[2].qnt >=1){
            this.invSun[2].qnt --
            this.spaces[id] = {id: id, state:"sun", value: this.invSun[2].value, end: this.invSun[2].value, item: "bg"}
                document.getElementById("simg"+id).setAttribute("src", `/nihches/bg.jpg`)
                this.saveTime()
          }
          break
          ...
          if(this.invSun[3].qnt >=1){
            this.invSun[3].qnt --
            this.spaces[id] = {id: id, state:"sun", value: this.invSun[3].value, end: this.invSun[3].value, item: "bh"}
                document.getElementById("simg"+id).setAttribute("src", `/nihches/bh.jpg`)
                this.saveTime()
          }
          break
    ...
    resetImg: function(){
      for(let i = 0; i <= this.spaces.length-1; i++){
        switch(this.spaces[i].item){
          ...
          case 7:
          case 8:
            document.getElementById("simg"+i).setAttribute("src", `/nihches/wh`+ ((this.spaces[i].item)-1) +`.jpg`)
          break  
          case "bds":
          case "invbds":
    ...
    whUP: function(){
      if(this.dm >= ((0.1*(Math.PI)**(this.whm+2))) && this.dm_p >= ((0.1*(Math.PI)**(this.whm+2)))){
